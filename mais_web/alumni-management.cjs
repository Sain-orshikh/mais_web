...
// Function to actually delete the files (only run with --confirm-delete)
function deleteUnselectedFiles() {
  try {
    const backupData = JSON.parse(fs.readFileSync('./alumni-backup-info.json', 'utf8'));
    const filesToRemove = backupData.jpgFilesToRemove;
    
    console.log('🗑️  Starting deletion of ' + filesToRemove.length + ' JPG files...');
    ...
    console.log('📁 Remaining JPG files: ' + backupData.jpgFilesToKeep.length);
    
  } catch (error) {
    console.error('❌ Error reading backup file:', error.message);
    console.log('Run the script without --confirm-delete first to generate the backup file.');
  }
}
...
module.exports = { main, deleteUnselectedFiles };
